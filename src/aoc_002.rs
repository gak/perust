...
mod tests {
    ...
    fn small() {
        ...
        assert_eq!(
            run(vec![2, 0, 0, 0, 2, 0, 0, 0, 99]),
            vec![16, 0, 0, 0, 2, 0, 0, 0, 99]
        );
    }
    ...
    fn part_two() {
        ...
        assert_eq!(which_output(data, 19690720), 3376);
    }
}
...
impl Program {
    ...
    fn ref_val(self: &Self, ptr: i32) -> i32 {
        self.0[self.0[ptr as usize] as usize]
    }

    fn set_val(self: &mut Self, pos: i32, val: i32) {
...
pub fn run(data: Vec<i32>) -> Vec<i32> {
    ...
    loop {
        match p.val(i) {
            99 => return p.0,
            1 => p.set_val(p.val(i + 3), p.ref_val(i + 1) + p.ref_val(i + 2)),
            2 => p.set_val(p.val(i + 3), p.ref_val(i + 1) * p.ref_val(i + 2)),
            _ => panic!("something went wrong at {}", i),
        }
        i += 4;
    }
}
